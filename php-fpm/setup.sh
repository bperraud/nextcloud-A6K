...
if [ ! -L config/config.php ]; then
    ...
    ./occ maintenance:install --data-dir "$datadir" --database mysql --database-name {{ index .Values "mariadb-galera" "db" "name" }} --database-user {{ index .Values "mariadb-galera" "db" "user" }} --database-pass "$DBPASS" --database-host proxysql --database-port 6033 --admin-user {{.Values.nextcloud.admin.user}} --admin-pass $NCADMINPASS
